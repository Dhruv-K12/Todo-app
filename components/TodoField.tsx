...
const TodoField = () => {
  ...
  const saveItems = async (data: taskType[]) => {
    await AsyncStorage.setItem("Items", JSON.stringify(data));
  }
  const addTaskHandler = () => {
    if (task.trim().length !== 0) {
      const userProps = {
        name: task, des: des, catogary: options, id: Math.random().toString(), isCompleted: false
      }
      setTasker(tasks => {
        saveItems([...tasks, userProps]);
        return [...tasks, userProps]
      });
      showModal(false);
      setTask('');
    }
